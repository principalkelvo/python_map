...
def to_mod_list(employee_list):
   ...
   employee_list=map(mod,employee_list)
   map_emp=[]
   print (employee_list)
...
def generate_usernames(mod_list):
   ...
   """
   ### WRITE SOLUTION CODE HERE       '
   mod_list = [w.replace(' ', '_') for w in mod_list]
...
#         print(str(i['id'])+ " : "+  str(i['name']))
   ...
   employees_initial = {x['name'][0]:x['id'] for x in employee_list}
   return(employees_initial)
